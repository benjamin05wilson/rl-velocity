...
run () {
  ...
  timeout 5400 "$RLV_PYTHON" -m rlv.train \
    ...
    --group-size "${GROUP}" \
    --seed "${seed}" \
    --rollout-repetition-penalty "${penalty}" --allow-off-policy \
    --eval-every "${EVAL_EVERY}" \
    --eval-prompts "${EVAL_PROMPTS}" \
    --run-name "${name}" \
    > "runs/sweep-logs/${name}.log" 2>&1
  echo "   exit=$? $(grep -c '^step' "runs/sweep-logs/${name}.log" 2>/dev/null) steps  $(date +%H:%M:%S)"
  grep '\[eval\]' "runs/sweep-logs/${name}.log" | tail -2
}
